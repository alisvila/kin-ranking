...
module.exports = async (message, client) => {
  ...
  const userData = await userRepData
    .findOne({
      GuildID: message.guild.id,
  ...
  if (message.channel.id === tradeRepChannel) {
    if (!userData) {
      ...
      newData.save();
      message.reply(
        `${message.mentions.members.first()}, You gained +1 Trade Rep!`
      );
    } else if (userData) {
      userData.TradeRep += 1;
      userData.save();
    ...
    if (!userData) {
      ...
      newData.save();
      message.reply(
        `${message.mentions.members.first()}, You gained +1 General Rep!`
      );
    } else if (userData) {
